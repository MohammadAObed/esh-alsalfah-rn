...
const PlayersImposterAnswer = () => {
  const { imposter, currentGame, gameAnswer, setStatus, setPlayers, players } = useGameContext();
  ...
  const checkAnswer = (imposterAnswerParam) => {
    const isCorrectAnswer = imposterAnswerParam.id == gameAnswer.id ? true : false;
          ...
          return (
            <TouchableOpacity
              className={`px-10 py-2.5 bg-red-800 rounded-sm ${
                ra.id == gameAnswer.id && imposterAnswer.id != 0 ? `bg-green-700` : ra.id == imposterAnswer.id ? `bg-red-900` : `bg-red-800`
              }`}
              // className="px-10 py-2.5 bg-red-800 rounded-sm"
              key={ra.id}
              onPress={() => {
            ...
            >
              <Text className="text-white text-3xl text-center">{Translator[language][ra.name]}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
